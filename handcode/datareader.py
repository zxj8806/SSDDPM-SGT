...
#     print(graph.y[sample_result])
  ...
  def tr_ts_val_numpy2dataloader(self, tr_mat, ts_mat, val_mat, tr_tag, ts_tag, val_tag, batch_size=64):
    ...
    # 创建节点ID的张量
    tr_nodes = torch.arange(tr_mat.size(0))  # 对应于tr_mat的行索引
    ts_nodes = torch.arange(ts_mat.size(0))  # 对应于ts_mat的行索引
    val_nodes = torch.arange(val_mat.size(0))  # 对应于val_mat的行索引

    # 创建带有节点ID的数据集
    train_dataset = Data.TensorDataset(tr_mat, tr_tag, tr_nodes)
    test_dataset = Data.TensorDataset(ts_mat, ts_tag, ts_nodes)
    val_dataset = Data.TensorDataset(val_mat, val_tag, val_nodes)
